import requests
import json
from bs4 import BeautifulSoup
import pandas as pd
import numpy as np
import time
import random

base_url = "https://api-web.nhle.com/v1/playoff-series/carousel/"
season_ext = ['19171918', '19181919', '19201921', '19211922', '19221923', '19231924', '19241925', '19251926', '19261927', '19271928',
              '19281929', '19291930', '19301931', '19311932', '19321933', '19331934', '19341935', '19351936', '19361937', '19371938',
              '19381939', '19391940', '19401941', '19411942', '19421943', '19431944', '19441945', '19451946', '19461947', '19471948',
              '19481949', '19491950', '19501951', '19511952', '19521953', '19531954', '19541955', '19551956', '19561957', '19571958',
              '19581959', '19591960', '19601961', '19611962', '19621963', '19631964', '19641965', '19651966', '19661967', '19671968',
              '19681969', '19691970', '19701971', '19711972', '19721973', '19731974', '19741975', '19751976', '19761977', '19771978',
              '19781979', '19791980', '19801981', '19811982', '19821983', '19831984', '19841985', '19851986', '19861987', '19871988',
              '19881989', '19891990', '19901991', '19911992', '19921993', '19931994', '19941995', '19951996', '19961997', '19971998',
              '19981999', '19992000', '20002001', '20012002', '20022003', '20032004', '20042005', '20052006', '20062007', '20072008',
              '20082009', '20092010', '20102011', '20112012', '20122013', '20132014', '20142015', '20152016', '20162017', '20172018',
              '20182019', '20192020', '20202021', '20212022', '20222023', '20232024', '20242025']

# season_ext = season_ext[-10:] # test only first season for now

seasonIDs = []; roundNums = []; seriesLetters = []; seriesLinks = []; bottomSeedIDs = []; bottomSeedWins = []
topSeedIDs = []; topSeedWins = []


for season in season_ext:
    url = f"{base_url}{season}"
    try:
        response = requests.head(url, allow_redirects=True)
        if response.status_code == 404:
            print("Page not found (404).")
        else:
            print("Page found.")
            data = requests.get(url).json()

            rounds = data['rounds']

            # iterate through 4 rounds (assuming past season)
            for round in rounds:
                seriesInfo = round['series']
                for series in seriesInfo:
                    seasonIDs.append(data['seasonId'])
                    roundNums.append(round['roundNumber'])
                    seriesLetters.append(series['seriesLetter'])
                    seriesLinks.append(series['seriesLink'])
                    bottomSeedIDs.append(series['bottomSeed']['id'])
                    bottomSeedWins.append(series['bottomSeed']['wins'])
                    topSeedIDs.append(series['topSeed']['id'])
                    topSeedWins.append(series['topSeed']['wins'])

    except requests.RequestException as e:
        print(f"Error fetching JSON: {e}")
        continue


rounds = data['rounds']


df = pd.DataFrame([seasonIDs, roundNums, seriesLetters, seriesLinks, bottomSeedIDs, 
                   bottomSeedWins, topSeedIDs, topSeedWins]).transpose()
df.columns = ['seasonIDs', 'roundNums', 'seriesLetters', 'seriesLinks', 'bottomSeedIDs', 
                   'bottomSeedWins', 'topSeedIDs', 'topSeedWins']

print(df.head())

df.to_csv("playoff_carousel.csv", index=False)