<header class="text-white shadow bg-slate-800">
  <div class="mx-auto max-w-7xl px-4 py-3 flex justify-between items-center">
    ...
    <nav id="nav-menu" class="hidden md:flex flex-col md:flex-row md:items-center gap-4 mt-4 md:mt-0 text-sm font-medium w-full md:w-auto">
      <!-- NHL DB dropdown -->
      <div class="relative">
        <a href="#" class="hover:text-blue-400 flex items-center space-x-1 dropdown-toggle">
          <span>NHL DB</span>
          <svg class="w-3 h-3 text-white transition" fill="currentColor" viewBox="0 0 20 20">
        ...
        <!-- Dropdown -->
        <ul class="hidden bg-slate-800 text-white rounded-md shadow-lg min-w-[180px] z-20 mt-2 flex-col dropdown-menu">
          <li><a href="nhlIndex.php" class="block px-4 py-2 hover:bg-slate-700">Game / Player Search (Home)</a></li>
          <li class="relative">
            <a href="#" class="block px-4 py-2 hover:bg-slate-700 dropdown-toggle">Teams ▸</a>
            <ul class="hidden bg-slate-800 text-white rounded-md shadow-lg min-w-[180px] z-30 mt-1 max-h-96 overflow-y-auto flex-col dropdown-menu">
              <!-- All teams -->
    ...
    document.querySelectorAll('.dropdown-toggle').forEach(toggle => {
      toggle.addEventListener('click', e => {
        ...
        if (nextMenu && nextMenu.classList.contains('dropdown-menu')) {
          nextMenu.classList.toggle('hidden');
        }
      });
    });
  </script>
